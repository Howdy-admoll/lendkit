...
class DocumentProcessor(ABC):
    ...
    @abstractmethod
    async def extract(
        self, storage_key: str, document_type: DocumentType
...
class TextractDocumentProcessor(DocumentProcessor):
    ...
    def __init__(self) -> None:
        try:
            ...
            raise RuntimeError(
                "boto3 is required for Textract provider. Run: pip install boto3"
            ) from None
    ...
    async def extract(
        ...
        response = self.textract.analyze_id(
            DocumentPages=[{"S3Object": {"Bucket": self.bucket, "Name": storage_key}}]
        )
        ...
        status = DocumentStatus.VERIFIED if normalized_conf >= 0.75 else DocumentStatus.REJECTED
