...
class KYCInitiateRequest(LendKitBaseModel):
    ...
    customer_id: str = Field(
        ..., min_length=1, max_length=64, description="Your internal customer UUID/ID"
    )
    tenant_id: str = Field(
        ..., min_length=1, max_length=64, description="Tenant / lender identifier"
    )
    ...
    date_of_birth: str | None = Field(
        None, pattern=r"^\d{4}-\d{2}-\d{2}$", description="ISO 8601 date"
    )
...
class DocumentUploadRequest(LendKitBaseModel):
    document_type: str = Field(
        ..., description="nin | bvn | passport | drivers_license | voters_card | utility_bill"
    )
